...
def get_health_advice(blood_pressure, blood_sugar, height, weight):
    ...
    if isinstance(blood_sugar, float):
        if blood_sugar > 140:
            ...
            fontSecondStrong['blood_pressure'].append("！注意！")
            fontNormalStrong['blood_pressure'].append("正常情況下，你的血糖應該為：")
            fontNormalStrong['blood_pressure'].append("空腹血糖：70~100mg/dL")
            fontNormalStrong['blood_pressure'].append("飯後兩小時血糖：70~140mg/dL")
            fontSecondStrong['blood_pressure'].append("！改善！")
            fontNormalStrong['blood_pressure'].append("1. 避免吃高升糖指數的食物，選擇 GI 值低於 55 的低升糖指數食物，例如：糙米、燕麥、山藥、大部分蔬菜、豆魚蛋肉類、小番茄、芭樂、木瓜等。")
            fontNormalStrong['blood_pressure'].append("1. 避免吃含精緻糖的食物，像是冰糖、砂糖、高果糖漿等精製糖，避免食用麵包、冰淇淋、甜甜圈等含精緻糖的食物。")
            fontNormalStrong['blood_pressure'].append("2. 避免吃高油脂食物，選擇植物油來替代動物性脂肪（如：豬油、牛油、奶油、培根等）。")
            fontNormalStrong['blood_pressure'].append("4. 避免吃高鈉食物，並避免過度飲酒。")
        elif blood_sugar < 70:
            ...
            fontNormalStrong['blood_pressure'].append("空腹血糖：70~100mg/dL")
            fontNormalStrong['blood_pressure'].append("飯後兩小時血糖：70~140mg/dL")
            fontNormalStrong['blood_pressure'].append("請注意您身理上是否有虛弱、嗜睡、飢餓、臉色蒼白、冒冷汗、心跳加快、發冷、抽筋、頭暈等症狀；心理上可能會有情緒改變及行為改變。")
            fontSecondStrong['blood_pressure'].append("！改善！")
            fontNormalStrong['blood_pressure'].append("以下為可能造成低血糖的原因：")
            fontNormalStrong['blood_pressure'].append("藥物：自行調整藥物劑量不正確、用藥時間不規律等。")
            fontNormalStrong['blood_pressure'].append("飲食：三餐不定時、用藥後未進食等。")
            fontNormalStrong['blood_pressure'].append("運動：空腹運動、過度運動等。")
            fontNormalStrong['blood_pressure'].append("如果長時間處於低血糖狀態，建議就醫。")
        else:
